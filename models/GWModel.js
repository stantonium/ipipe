...
async function GWModel(data) {
	...
	tab += " <th>Open Water Evaporation (mm)</th> <th>Snowfall (cm)</th> <th>Snow Depth (cm)</th> <th>Net Radiation</th>";
	tab += "</tr> </thead><tbody>";
	...
	while (x < len) {
		...
		if (data[x].REPORT_TYPE == "FM-15" || selectedStation == '08383099999' || selectedStation == '08384099999') {
			...
			if (minutes <= 50 && minutes >= 10) { null } else {
				if (data[x].REPORT_TYPE == "FM-15") {
					...
					if (ic1 == null) {
						...
						let SRA = (12 / Math.PI) * 4.92 * DR * ((STAE - STAB) * Math.sin(lat) * Math.sin(DECL) + Math.cos(lat) * Math.cos(DECL) * (Math.sin(STAE) - Math.sin(STAB)));
						let SRS = ((0.25 + 0.50 * (1.0 - ga1 * 0.01)) + 0.00002 * ELEV) * SRA;
						let SRS_CS = ((0.25 + 0.50 * (1.0 - 0 * 0.01)) + 0.00002 * ELEV) * SRA;
						let SUNA = Math.asin(Math.sin(lat)) * Math.sin(DECL) + Math.cos(lat) *  Math.cos(DECL) * Math.cos(STAM);
						let SUNRAT = 0;
						if (SUNRAT < 0.3) {
							SUNRAT = 0.55
						} else if (SUNA > 1.0) {
							SUNRAT = 1.0
						} else {SUNRAT = SRS/SRS_CS}
						...
						let VP = 6.108 * Math.pow(Math.E, (17.269 * dew / (dew + 237.3)));
						let SVP = 6.108 * Math.pow(Math.E, (17.27 * tmp / (tmp + 237.3)));
						let RNL = (2.042* Math.pow(2.042, -10)) * DCF * (0.34 - 0.14 * Math.sqrt(VP)) * Math.pow((tmp + 273.16), 4);
						let RNET = RNS - RNL;
						var RNSC;
						var RNLC;
						...
						let AERO = 0.268 * (0.5 + 0.54 * WSPH) * (SVP/10 - VP);
...
function GWTable(data, metadata) {
	...
	while (x < len) {
		...
		tab += "<td>" + data[x].Net_Radiation + "</td>"; //Net Radiation

		tab += "</tr>";
		//console.log(tab);
		//alert("Stop here for a second");
		x++;
	}
